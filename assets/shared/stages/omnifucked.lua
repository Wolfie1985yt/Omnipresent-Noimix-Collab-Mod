...
function onCreate()
	...
	precacheSong('omnipresent-fucked/Inst');
...
function onUpdate(elapsed)
	...
	if curStep == 1040 or curStep == 1296 or curStep == 1552 or curStep == 1808 or curStep == 10416 then --Needlemouse-p2
		runTimer('ringStart',0.1);
		runTimer('ringFinish',0.2);
		xx = 550;
		yy = 680;
	...
	if curStep == 12368 then --Needlemouse-p2 final moment
...
function onTimerCompleted(tag, loops, loopsLeft)
	...
	if tag == 'preload' and getPropertyFromClass('states.PlayState','chartingMode') == false and getPropertyFromClass('backend.ClientPrefs','data.potatoMode') == false then
		...
		triggerEvent('Change Character', 'dad', 'Needlemouse-p2');
		...
		triggerEvent('Change Character', 'dad', 'Needlemouse-p2-flipped');
		...
		triggerEvent('Change Character', 'gf', 'Needlemouse-p2');
		...
		triggerEvent('Change Character', 'gf', 'Needlemouse-p2-flipped');
		...
		triggerEvent('Change Character', 'bf', 'Needlemouse-p2');
		...
		triggerEvent('Change Character', 'bf', 'Needlemouse-p2-flipped');
...
function opponentNoteHit(id, direction, noteType, isSustainNote, gfSection)
	if getProperty('health') > 0.1 and not isSustainNote then
...
function goodNoteHit(id, direction, noteType, isSustainNote, gfNote)
	if exeMoment and getProperty('health') > 0.1 and not isSustainNote then
