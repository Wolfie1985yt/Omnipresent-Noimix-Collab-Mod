...
class OmniFucked extends BaseStage
{
	...
	override function create()
	{
		...
		if (ClientPrefs.data.smartCache && !lowQuality && !lowerQuality || lowerQuality) {
			gpuCache = true;
		}
		//Gpu stuff
		if (gpuCache && !lowQuality)
		{
		...
		preload('characters/NeedlemouseSarahShadowmouseLutherSunter/Needlemouse-p2');
		preload('characters/BF/HD/needle-bf-encore');
		preload('characters/Sanic/sanic');
		preload('characters/LordX/lordxEncore');
		preload('characters/Majin/SonicFunAssets');
		...
		if (!lowQuality && !lowerQuality || lowerQuality) {
			...
			preload('bgs/needlemouse-p2/needlemouse2LowQuality');
			preload('bgs/sanic/sanicbg');
		...
		if (!lowQuality && !lowerQuality || lowerQuality) {
			...
			conkCreet = new BGSprite('bgs/needlemouse-p2/needlemouse2LowQuality', -775, -141, 1, 0.9);
			conkCreet.visible = false;
			add(conkCreet);
			...
			tails_floor = new FlxSprite(-600, -900);
